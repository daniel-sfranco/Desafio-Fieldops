...
from sqlalchemy import (
    String, Enum as SQLEnum, Text, ForeignKey, DateTime, Integer
)
...
class OS(base):
    ...
    assigneeId: Mapped[Optional[int]] = mapped_column(
        ForeignKey("flx_users.id"), nullable=True
    )
    ...
    assignee: Mapped[Optional["Usuario"]] = relationship(
        "Usuario", back_populates="osList"
    )
    checkList: Mapped[List["Checklist"]] = relationship(
        "Checklist", back_populates="workOrder", cascade="all, delete-orphan"
    )
    auditList: Mapped[List["Auditoria"]] = relationship(
        "Auditoria", back_populates="workOrder", cascade="all, delete-orphan"
    )
